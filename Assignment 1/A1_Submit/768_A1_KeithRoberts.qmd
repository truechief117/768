...
Recall that the Stein effect refers to a phenomenon where certain shrinkage estimators outperform traditional unbiased estimators, in terms of mean squared error (MSE). Here, the SMSE-optimal lambdaestimators introduce bias through smoothing (shrinkage toward more central probability values). Checking the values of the SMSE estimators computed above, we note that across all estimators the SMSE is indeed lower than the "true probability" model only in some cases we analyzed. Thus, there might be a small Stein effect at work but it seems that the result is negligible. These values can be shown in the tables below:
